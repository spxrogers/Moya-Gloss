...
public extension ObservableType where E == Response {
  ...
  /// Observable .Errors's on failure.
  public func mapObject<T: Decodable>(_ type: T.Type, forKeyPath keyPath: String) -> Observable<T> {
    return flatMap { response -> Observable<T> in
  ...
  /// Observable .Errors's on failure.
  public func mapArray<T: Decodable>(_ type: T.Type) -> Observable<[T]> {
    return flatMap { (response) -> Observable<[T]> in
  ...
  /// Observable .Errors's on failure.
  public func mapArray<T: Decodable>(_ type: T.Type, forKeyPath keyPath: String) -> Observable<[T]> {
    return flatMap { (response) -> Observable<[T]> in
